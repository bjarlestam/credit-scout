...
def parse_analysis_results(analysis_text: str) -> Dict:
    ...
    # Extract intro end time (handle both markdown and plain text formats)
    intro_match = re.search(r"(?:\*\*)?Intro ends at:?\*?\*?\s*(\d{1,2}:\d{2})", analysis_text, re.IGNORECASE)
    if intro_match:
        results["intro_end_time"] = intro_match.group(1)
    
    # Extract outro start time (handle both markdown and plain text formats)
